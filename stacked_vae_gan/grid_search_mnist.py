    ...
    for learning_rate in [1E-4, 1E-5, 1E-6]:
        for latent_dim in [20, 50, 100, 128]:
            for clipped_gradient in [True, False]:
                ...
                root_log_dir = "./runtime/logs/mnist_vaegan/lr_%s_z_%d_gp_%s" % (learning_rate, latent_dim, clipped_gradient)
                vaegan_checkpoint_dir = "./runtime/models/mnist_vaegan/lr_%s_z_%d_gp_%s/mnist_model.ckpt" % (learning_rate, latent_dim, clipped_gradient)
                sample_path = "./runtime/samples/mnist_vaegan/lr_%s_z_%d_gp_%s" % (learning_rate, latent_dim, clipped_gradient)
