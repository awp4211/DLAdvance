...
class vaegan_mnist(object):
    ...
    def build_model_vaegan(self):
        ...
        #For encode
        self.encode_loss = self.kl_loss/(self.batch_size*self.latent_dim) - self.LL_loss
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            sess.run(init)
            self.saver.restore(sess, self.saved_model_path)
            realbatch_array = self.data[0: self.batch_size]

            real_images, sample_images = sess.run([self.images, self.x_tilde], feed_dict={self.images: realbatch_array})
            save_images(sample_images[0:self.batch_size], [int(np.sqrt(self.batch_size)), int(np.sqrt(self.batch_size))],
                        '{}/train_{:02d}_{:04d}_con.png'.format(self.sample_path, 0, 0))
            save_images(real_images[0:self.batch_size], [int(np.sqrt(self.batch_size)), int(np.sqrt(self.batch_size))],
