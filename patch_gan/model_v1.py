...
class PatchGAN(object):
    ...
    def build_model(self):
        ...
        self.G = tf.add(tf.add(tf.add(g_0, g_1), g_2), g_3)
    ...
    def sampler(self, z, y=None):
        ...
        for index in range(self.num_patches):
            with tf.variable_scope("generator%d" % index) as scope:
                scope.reuse_variables()
                yb = tf.reshape(y[index], [self.batch_size, 1, 1, self.y_dim+2])
        ...
        hh_0 = tf.pad(h[0], paddings=[[0, 0], [0, self.output_height / 2], [0, self.output_width / 2], [0, 0]], mode="CONSTANT")
        hh_1 = tf.pad(h[1], paddings=[[0, 0], [self.output_height / 2, 0], [0, self.output_width / 2], [0, 0]], mode="CONSTANT")
        hh_2 = tf.pad(h[2], paddings=[[0, 0], [0, self.output_height / 2], [self.output_width / 2, 0], [0, 0]], mode="CONSTANT")
        hh_3 = tf.pad(h[3], paddings=[[0, 0], [self.output_height / 2, 0], [self.output_width / 2, 0], [0, 0]], mode="CONSTANT")
