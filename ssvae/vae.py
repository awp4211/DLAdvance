...
class VariationalAutoencoder(object):
    ...
    def __init__(self, dim_x, dim_z,
                 p_x='bernoulli', q_z='gaussian_marg', p_z='gaussian_marg',
                 hidden_layers_px=[600, 600], hidden_layers_qz=[600, 600],
                 nonlin_px=tf.nn.softplus, nonlin_qz=tf.nn.softplus,
                 l2_loss=0.0):
    ...
    def _objective(self):
        ...
        self.x_recon, self.recon_logits = self._generate_xz(self.z_sample)
    ...
    def train(self, x, x_valid, epochs, num_batches, print_every=1,
              learning_rate=3e-4, beta1=0.9, beta2=0.999, seed=31415,
              stop_iter=100, save_path=None, load_path=None, draw_img=1):
        self.num_examples = x.shape[0]
        self.num_batches = num_batches

        assert self.num_examples % self.num_batches == 0

        self.batch_size = self.num_examples // self.num_batches

        if save_path is None:
            self.save_path = 'checkpoints/model_VAE_{}-{}.cpkt'.format(learning_rate, self.batch_size)
        else:
                    ...
                    if draw_img > 0:
                        ...
                        for i, row in enumerate(axes):
                            ...
                            im = row[1].imshow(x_recon_sample[i].reshape(28, 28), vmin=0, vmax=1,
                                                   cmap=sns.light_palette((1.0, 0.4980, 0.0549), input="rgb",
                                                                          as_cmap=True))
